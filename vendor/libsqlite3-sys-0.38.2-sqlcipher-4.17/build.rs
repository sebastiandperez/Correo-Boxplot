...
fn main() {
    ...
    if cfg!(feature = "bundled-sqlcipher") {
        if env::var_os("LIBSQLITE3_SYS_USE_PKG_CONFIG").is_some_and(|value| value != "0") {
            panic!("pinned SQLCipher builds forbid LIBSQLITE3_SYS_USE_PKG_CONFIG");
        }
        if cfg!(feature = "bundled-sqlcipher-vendored-openssl")
            && env::var_os("OPENSSL_NO_VENDOR").is_some_and(|value| value != "0")
        {
            panic!("pinned SQLCipher builds forbid OPENSSL_NO_VENDOR");
        }
    }
