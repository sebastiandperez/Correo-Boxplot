...
export function loadPersistedEmailsByFolder(): Record<string, Email[]> {
  ...
  try {
    ...
    for (const [folderKey, emailList] of Object.entries(parsed)) {
      result[folderKey] = emailList.map((e) =>
        email({
          ...e,
          keywords: new Set(e.keywords ?? []),
        }),
      )
    }
    return result
    ...
    console.warn(
      '[mock-data] Falló lectura de localStorage, usando defaults:',
      err,
    )
    return createMockEmailsByFolder()
  }
...
export function savePersistedEmailsByFolder(
  ...
  try {
    ...
    console.warn('[mock-data] Falló escritura en localStorage:', err)
  }
}
