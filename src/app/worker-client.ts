...
export class JmapWorkerClient {
  ...
  constructor() {
    this.worker = new Worker(new URL('../workers/jmap-worker.ts', import.meta.url), { type: 'module' })
  ...
  syncAccount(accountKey: AccountKey, jmapAccountId: string, sinceState: string) {
  ...
  sendEmail(accountKey: AccountKey, jmapAccountId: string, mutation: SendMutation) {
    const msg: MainToWorkerMessage = {
      type: 'SEND_EMAIL',
