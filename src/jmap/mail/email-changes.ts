...
export async function getEmailChanges(
  ...
  try {
    const requestResult = await jam.request(['Email/changes', {
      accountId,
      sinceState,
    }])
    response = requestResult[0] as RawJmapChangesResponse
  } catch (err: unknown) {
    throw new JmapMethodError('Email/changes', 'networkOrServerFail', err instanceof Error ? err.message : String(err))
  }
  ...
  if ('cannotCalculateChanges' in response) {
    throw new JmapMethodError('Email/changes', 'cannotCalculateChanges', 'Server cannot calculate changes from this state.')
  }
