...
describe('JMAP Submission', () => {
  ...
  it('should successfully batch create and submit email', async () => {
    const mockJam = {
      request: vi.fn()
        ...
        .mockResolvedValueOnce([
          ...
          ['EmailSubmission/set', { created: { 'sub1': { id: 'real-sub-id' } } }, 's1']
        ])
    ...
    const batchCall = vi.mocked(mockJam.request).mock.calls[1][0] as any
  ...
  it('should throw JmapMethodError if email creation is too large', async () => {
    const mockJam = {
      request: vi.fn()
        ...
        .mockResolvedValueOnce([
          ['Email/set', { notCreated: { 'draft1': { type: 'tooLarge', description: 'Too big' } } }, 'e1'],
  ...
  it('should throw if Drafts mailbox is not found', async () => {
    ...
    await expect(submitEmail(mockJam, 'acc1', dummyIntent, 'raw-id-123'))
