...
describe('JMAP Mail APIs', () => {
  ...
  it('queryEmails should support pagination', async () => {
    ...
    const result = await queryEmails(mockJam, 'acc1', 'mb1', { unread: true }, { limit: 10, position: 0 })
    expect(mockJam.request).toHaveBeenCalledWith(['Email/query', {
      accountId: 'acc1',
  ...
  it('queryAndGetEmails should construct batch refs', async () => {
    ...
    const result = await queryAndGetEmails(mockJam, 'acc1', 'mb1', null, { limit: 5 })
    
    expect(mockJam.request).toHaveBeenCalledTimes(1)
    const calls = vi.mocked(mockJam.request).mock.calls[0][0] as any
