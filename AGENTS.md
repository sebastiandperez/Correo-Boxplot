...
Este repositorio contiene el **cliente** de un sistema de correo local-first cliente/servidor. El servidor vive aparte y está fuera de alcance aquí — no implementes lógica de servidor, IMAP/SMTP **entrante**, ni nada que corresponda al VPS. El cliente remoto se rige por ADR-008: JMAP, y en el futuro IMAP/SMTP, quedan detrás de `RemoteMail` + `Submission`.
...
- **Protocolo hacia el servidor:** dos protocolos obligatorios, JMAP real sobre HTTPS + WebSocket para push, e IMAP/SMTP. Coordinator depende de `RemoteMail`; Outbox depende de `RemoteMail` + `Submission`. `JmapClient` es API interna del adapter JMAP, no el puerto universal. No implementes un RPC propio ni un tercer protocolo.
- **Runtime de red/sync:** Cliente JMAP, Coordinator y Outbox son TypeScript en un Worker normal del webview Tauri. JMAP usa `fetch`/WebSocket directo. Rust administra SQLite, SQLCipher y secretos locales; cuando se implemente IMAP/SMTP podrá abrir su TCP/TLS nativo detrás de adapters protocol-neutral, pero nunca fabricará DTOs JMAP ni actuará como proxy genérico (ADR-008).
...
- Las futuras credenciales IMAP/SMTP (ADR-008) vivirán solo en memoria del proceso Rust; nunca se persistirán, volverán por IPC hacia TypeScript ni aparecerán en logs. La futura capa nativa de protocolo será la única que abra esos sockets y nunca adquirirá el `EngineLease`. Sin TLS solo se permitirá contra `127.0.0.1`/`localhost`; cualquier otro host exigirá TLS y fallará cerrado.
...
- Los adaptadores TypeScript Tauri satisfacen `ReadRepository`/`SyncPort` y concentran `invoke()`; Rust implementa su semántica de persistencia y nunca habla JMAP. Una futura capa nativa IMAP/SMTP (ADR-008) será distinta del motor, no conocerá sus Ports y no traducirá a JMAP.
...
- Dependencias Rust futuras para IMAP/SMTP/TLS (ADR-008) se fijan con versión exacta y se ejecutan con `--locked`, igual que el resto de `Cargo.toml`.
