...
`SyncPort` P-02 está cerrado como frontera de transiciones semánticas atómicas consumida por casos de escritura de Application, Coordinator y Outbox. `LocalChangeSource` P-03 está implementado, con review pendiente, como port separado para invalidaciones post-commit: sus señales no son autoridad, pueden coalescerse o duplicarse y hacen que el consumidor relea mediante `ReadRepository`. Las solicitudes `ensure…` o de materialización remota pertenecen a futura orquestación Application → Coordinator, no a ninguno de estos ports.
...
| `src/ports/` | Contratos locales | P-01 y P-02 cerrados; P-03 implementado/review pendiente; audit de Ports pendiente |
