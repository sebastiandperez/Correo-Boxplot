...
*   **Orquestador de sincronización y concurrencia:**
    ...
    *   En Tauri corre en un Worker normal: `JmapRemoteMail`/`JmapSubmission` reutilizan el cliente JMAP por `fetch`/WebSocket. Los adapters IMAP/SMTP y su transporte nativo están diferidos.
    *   Rust Local Engine nunca habla JMAP ni aloja Coordinator/Outbox. Una futura capa nativa IMAP/SMTP permanecerá separada del motor y no fabricará DTOs JMAP.
...
*   **Protocolo hacia el servidor:** JMAP e IMAP implementan `RemoteMail`; JMAP y SMTP implementan `Submission`. El Worker habla JMAP directamente; IMAP/SMTP siguen diferidos. Los estados remotos son opacos y ningún tipo concreto cruza hacia Coordinator/Outbox.
